...
This tutorial will help you setup a basic 3D scene in Android using the Rajawali 3D library. It is assumed that you are familiar with configuring a project to use Rajawali. If you are not, please see the above mentioned tutorial or the [Installation Guide](https://github.com/Rajawali/Rajawali/blob/master/docs/installation_guide.md).

## Setting Up Our Scene

We are going to make a simple scene of the Earth rotating. First, we need to extend the `RajawaliRenderer` class. Let’s create a new class in the same package as our `MainActivity` and name it **Renderer**.
